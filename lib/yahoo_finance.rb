...
class YahooFinance
  ...
  class << self
    def company_metrics(symbols_array, columns_array = COLUMNS, options = { })
    # binding.pry
      options[:raw] ||= true
      ret = []
      symbols_array.each_slice(SYMBOLS_PER_REQUEST) do |symbols|
        get_metrics(symbols.join("+"), columns_array).map do |row|
        hash = row.to_hash
        hash.delete(nil)
    ...
    def historical_stock_prices(symbol, start_date, end_date, options = {})
    options[:raw] ||= true
    options[:period] ||= :daily
    read_historicals(symbol, start_date, end_date, options).map do |row|
      binding.pry
    ...
    text = conn.read.gsub(Regexp.new('\"'),'""')
    CSV.parse(text, :headers => cols)
    end

    def read_historicals(symbol, start_date, end_date, options)
    ...
    conn = open(url)
    cols = if options[:dividends_only]
         [:dividend_pay_date, :dividend_yield]
         else
         [:trade_date, :volume, :adjusted_close]
         end

    text   = conn.read.gsub(Regexp.new('\"'),'""')
    result = CSV.parse(text, :headers => cols) #:first_row, :header_converters => :symbol)
